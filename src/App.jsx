    ...
    <main className="relative z-10 container mx-auto px-4 pb-4 flex flex-col items-center -mt-4">
      ...
      <footer className="mt-4 pb-8 text-center text-gray-500 text-sm font-medium">
        <div className="flex items-center justify-center gap-2">
          ...
          <a href="https://www.facebook.com/ikbal.ouaissa" target="_blank" rel="noreferrer" className="text-[#009FE3] font-bold flex items-center gap-1">
  ...
  <div className="min-h-screen flex items-center justify-center bg-gray-50 relative font-sans" dir="rtl">
    <div className="bg-white/80 backdrop-blur-xl p-12 rounded-[2.5rem] shadow-2xl w-full max-w-[480px] border border-white/50 z-10">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-500 mt-2 text-sm">استعمل بريد المشرف وكلمة المرور (Firebase)</p>
      </div>
...
const DashboardView = ({
  ...
  const availableCities = useMemo(() => [...new Set(registrations.map(r => r.city).filter(Boolean))].sort(), [registrations]);
  const availableRegions = useMemo(() => [...new Set(registrations.map(r => r.region).filter(Boolean))].sort(), [registrations]);
  ...
  const filteredRegistrations = useMemo(() => {
    return registrations.filter(item => {
      ...
      const matchSearch = s
        ? ((item.fullName || '').toLowerCase().includes(s) || (item.phone || '').includes(s))
        : true;
  ...
  const regionData = useMemo(() => {
    const c = {};
    filteredRegistrations.forEach(r => c[r.region] = (c[r.region] || 0) + 1);
    return Object.keys(c).map(k => ({ name: k, value: c[k] })).sort((a, b) => b.value - a.value);
  }, [filteredRegistrations]);

  const professionData = useMemo(() => {
    const c = {};
    filteredRegistrations.forEach(r => c[r.profession] = (c[r.profession] || 0) + 1);
    return Object.keys(c).map(k => ({ name: k, value: c[k] })).sort((a, b) => b.value - a.value);
  }, [filteredRegistrations]);

  const cityData = useMemo(() => {
    const c = {};
    filteredRegistrations.forEach(r => c[r.city] = (c[r.city] || 0) + 1);
    return Object.keys(c).map(k => ({ name: k, value: c[k] })).sort((a, b) => b.value - a.value);
  }, [filteredRegistrations]);
    ...
    <div className="min-h-screen bg-slate-100 font-sans flex text-right" dir="rtl">
      <aside className={`bg-[#1e293b] text-white transition-all duration-500 fixed h-full z-20 shadow-2xl ${sidebarOpen ? 'w-72' : 'w-24'} flex flex-col`}>
        ...
        <div className="p-6 mt-auto">
          <button onClick={onLogout} className="text-red-400 flex gap-4">
            <LogOut />{sidebarOpen && "خروج"}
          </button>
        </div>
      </aside>
      ...
      <div className={`flex-1 transition-all duration-500 ${sidebarOpen ? 'mr-72' : 'mr-24'}`}>
        <header className="bg-white shadow-sm sticky top-0 z-10 p-5 flex justify-between">
          ...
          <div className="text-[#009FE3] font-bold">المشرف: {loginUserLabel}</div>
        </header>
        ...
        <main className="p-10 space-y-8">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm">
            <div className="flex flex-col lg:flex-row gap-4 justify-between items-end mb-4">
              ...
              <div className="flex gap-2 w-full lg:w-auto">
                <button onClick={() => exportToExcel(filteredRegistrations)} className="flex-1 bg-green-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-green-700 flex items-center justify-center gap-2">
                  <Download size={18} /> Excel
                </button>
                <button onClick={exportToPDF} className="flex-1 bg-red-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-red-700 flex items-center justify-center gap-2">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm flex justify-between">
              <div>
                <p className="font-bold text-gray-500">مجموع المسجلين</p>
                <h3 className="text-5xl font-black text-[#009FE3]">{registrations.length}</h3>
              </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm flex justify-between">
              <div>
                <p className="font-bold text-gray-500">الجهات المغطاة</p>
                <h3 className="text-5xl font-black text-purple-600">{regionData.length}</h3>
              </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm flex justify-between">
              <div>
                <p className="font-bold text-gray-500">المدن المغطاة</p>
                <h3 className="text-5xl font-black text-indigo-600">{cityData.length}</h3>
              </div>
...
export default function App() {
  ...
  // المصادقة: إذا لا يوجد مستخدم => دخول مجهول للزوار
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      async (err) => {
        ...
        // إذا ليست لديك صلاحية (ليس admin) ستظهر هنا غالباً
  ...
  const handleSubmit = async (e) => {
    ...
    if (!auth.currentUser) {
      alert("جاري تهيئة الاتصال... أعد المحاولة بعد ثوانٍ.");
      return;
    }
  ...
  // ✅ دخول المشرف الحقيقي عبر Firebase (Email/Password)
  const handleAdminLogin = async (e) => {
    e.preventDefault();
    setLoginError('');

    try {
      await signOut(auth).catch(() => {}); // تأكد لا يبقى anonymous
      await signInWithEmailAndPassword(auth, loginUser, loginPass);
      setView('dashboard');
    } catch (err) {
      console.error("Admin login error:", err);
      setLoginError("بيانات الدخول غير صحيحة أو لا يمكن تسجيل الدخول.");
    }
  };
  ...
  const loginUserLabel = loginUser || (user?.email || "admin");
