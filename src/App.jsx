...
import {
  ...
  AlertCircle, ChevronDown, Lock, Menu, Image as ImageIcon,
  Facebook, Building, Search, FileText
} from 'lucide-react';
  ...
  <div className="min-h-screen bg-slate-50 relative overflow-hidden font-sans" dir="rtl">
    ...
    <header className="relative z-10 container mx-auto px-4 py-6 flex justify-between items-center">
      ...
      <button
        ...
        className="text-white hover:bg-white/20 transition-all flex items-center gap-2 px-5 py-2 rounded-full border border-white/20 backdrop-blur-md shadow-lg group"
      >
    ...
    <main className="relative z-10 container mx-auto px-4 pb-4 flex flex-col items-center -mt-4">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden border border-gray-100 relative">
        ...
        {submitStatus === 'success' ? (
          <div className="p-16 text-center animate-fade-in">
            <div className="w-28 h-28 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl">
              <CheckCircle size={56} />
            </div>
            <h2 className="text-4xl font-black text-gray-800 mb-4">تم التسجيل بنجاح!</h2>
            <button
              onClick={() => setSubmitStatus('idle')}
              className="bg-[#009FE3] text-white px-10 py-4 rounded-2xl font-bold hover:bg-blue-600 shadow-xl mx-auto flex gap-2"
            >
              تسجيل عضو جديد <ChevronDown className="rotate-270" />
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="px-8 pb-12 md:px-20 pt-4 space-y-8">
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-[#1e3a8a] flex items-center gap-3 border-b border-blue-100 pb-3">
                <User className="text-[#009FE3]" /> المعلومات الشخصية
              </h3>
              <div className="grid md:grid-cols-2 gap-4">
                <input name="fullName" value={formData.fullName} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none"
                  placeholder="الاسم الكامل" required
                />
                <input name="cnie" value={formData.cnie} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none uppercase"
                  placeholder="CNIE" required
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-[#1e3a8a] flex items-center gap-3 border-b border-blue-100 pb-3">
                <Phone className="text-[#009FE3]" /> التواصل
              </h3>
              <div className="grid md:grid-cols-2 gap-4">
                <input name="phone" value={formData.phone} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none"
                  placeholder="رقم الهاتف (10 أرقام)" dir="ltr" maxLength="10" required
                />
                <input name="email" type="email" value={formData.email} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none"
                  placeholder="البريد الإلكتروني" dir="ltr" required
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-[#1e3a8a] flex items-center gap-3 border-b border-blue-100 pb-3">
                <Briefcase className="text-[#009FE3]" /> المهنة والجهة
              </h3>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="relative">
                  <select name="region" value={formData.region} onChange={handleInputChange}
                    className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none appearance-none cursor-pointer"
                    required
                  >
                ...
                <input name="city" value={formData.city} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none"
                  placeholder="المدينة / المديرية الإقليمية" required
                />
                <input name="profession" value={formData.profession} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none"
                  placeholder="المهنة (مثال: أستاذ التعليم الابتدائي)" required
                />
                <input name="province" value={formData.province} onChange={handleInputChange}
                  className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:bg-white border-gray-100 rounded-2xl focus:border-[#009FE3] outline-none"
                  placeholder="الإقليم" required
                />
              </div>
            </div>

            <button type="submit" disabled={loading}
              className="w-full bg-gradient-to-r from-[#009FE3] to-[#1e3a8a] text-white font-black text-xl py-5 rounded-2xl shadow-xl hover:shadow-2xl transition-all flex items-center justify-center gap-4 hover:scale-[1.01]"
            >
      ...
      <footer className="mt-4 pb-8 text-center text-gray-500 text-sm font-medium">
        <div className="flex items-center justify-center gap-2">
          ...
          <a href="https://www.facebook.com/ikbal.ouaissa" target="_blank" className="text-[#009FE3] font-bold flex items-center gap-1">
            ذ.إقبال أوعيسى <Facebook size={14} />
          </a>
        </div>
      </footer>
    </main>
...
// --- 2. واجهة الدخول (Firebase Email/Password) ---
const LoginView = ({ handleAdminLogin, loginEmail, setLoginEmail, loginPass, setLoginPass, loginError, setView }) => (
  <div className="min-h-screen flex items-center justify-center bg-gray-50 relative font-sans" dir="rtl">
    <div className="bg-white/80 backdrop-blur-xl p-12 rounded-[2.5rem] shadow-2xl w-full max-w-[480px] border border-white/50 z-10">
      <div className="text-center mb-12">
        <Logo size="small" />
        <h2 className="text-3xl font-black text-gray-800 mt-8">بوابة المشرفين</h2>
        <p className="text-gray-500 mt-2 text-sm">مرحبا بك</p>
      </div>
      ...
      <form onSubmit={handleAdminLogin} className="space-y-6">
        ...
        <button type="submit" className="w-full bg-[#1e293b] text-white py-5 rounded-2xl font-black hover:bg-black transition-all shadow-xl">
          دخول
        </button>
        <button type="button" onClick={() => setView('form')} className="w-full text-gray-400 font-bold">
          عودة
        </button>
      </form>
    </div>
...
// --- 3. لوحة التحكم ---
const DashboardView = ({ sidebarOpen, setSidebarOpen, setView, adminEmail, exportToExcel, exportToPDF, registrations, onLogout }) => {
  const [filterRegion, setFilterRegion] = useState('');
  const [filterCity, setFilterCity] = useState('');
  const [searchTerm, setSearchTerm] = useState('');

  const availableCities = useMemo(() => [...new Set(registrations.map(r => r.city).filter(Boolean))], [registrations]);
  const availableRegions = useMemo(() => [...new Set(registrations.map(r => r.region).filter(Boolean))], [registrations]);
    ...
    <div className="min-h-screen bg-slate-100 font-sans flex text-right" dir="rtl">
      <aside className={`bg-[#1e293b] text-white transition-all duration-500 fixed h-full z-20 shadow-2xl ${sidebarOpen ? 'w-72' : 'w-24'} flex flex-col`}>
        ...
        <div className="p-6 mt-auto space-y-4">
          <button onClick={onLogout} className="text-white/80 hover:text-white flex gap-4">
            <LogOut />{sidebarOpen && "تسجيل الخروج"}
          </button>
          <button onClick={() => setView('form')} className="text-red-400 flex gap-4">
            <LogOut />{sidebarOpen && "خروج إلى الاستقبال"}
          </button>
        </div>
      </aside>
      ...
      <div className={`flex-1 transition-all duration-500 ${sidebarOpen ? 'mr-72' : 'mr-24'}`}>
        <header className="bg-white shadow-sm sticky top-0 z-10 p-5 flex justify-between">
          ...
          <div className="text-[#009FE3] font-bold" dir="ltr">Admin: {adminEmail}</div>
        </header>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm">
            <div className="flex flex-col lg:flex-row gap-4 justify-between items-end mb-4">
              <div className="flex gap-4 w-full lg:w-3/4">
                <div className="flex-1">
                  ...
                  <div className="relative">
                    <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full bg-gray-50 border rounded-xl p-3 pl-10 focus:border-[#009FE3] outline-none"
                      placeholder="بحث..."
                    />
                ...
                <div className="flex-1">
                  ...
                  <select value={filterRegion} onChange={(e) => setFilterRegion(e.target.value)}
                    className="w-full bg-gray-50 border rounded-xl p-3 outline-none"
                  >
              ...
              <div className="flex gap-2 w-full lg:w-auto">
                <button onClick={() => exportToExcel(filteredRegistrations)}
                  className="flex-1 bg-green-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-green-700 flex items-center justify-center gap-2"
                >
                  <Download size={18} /> Excel
                </button>
                <button onClick={exportToPDF}
                  className="flex-1 bg-red-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-red-700 flex items-center justify-center gap-2"
                >
            ...
            <div className="text-sm text-gray-500 font-bold">
              عدد النتائج: <span className="text-[#009FE3] text-lg">{filteredRegistrations.length}</span>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8 h-96">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm lg:col-span-1 flex flex-col">
              ...
              <div className="flex-1 min-h-0">
                <ResponsiveContainer>
                  <PieChart>
                    <Pie data={regionData} dataKey="value" outerRadius={80} label>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm lg:col-span-1 flex flex-col">
              ...
              <div className="flex-1 min-h-0">
                <ResponsiveContainer>
                  <BarChart data={professionData.slice(0, 5)} layout="vertical">
                    <XAxis type="number" hide />
                    <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 12 }} />
                    <Tooltip />
                    <Bar dataKey="value" fill="#009FE3" barSize={20} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="flex-1 min-h-0">
                <ResponsiveContainer>
                  <BarChart data={cityData.slice(0, 5)} layout="vertical">
                    ...
                    <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 12 }} />
                    <Tooltip />
                    <Bar dataKey="value" fill="#1e3a8a" barSize={20} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
...
export default function App() {
  ...
  // 2) Check admin role when user changes
  useEffect(() => {
    const check = async () => {
      ...
      try {
        ...
        // إذا القواعد تمنع القراءة أو الوثيقة غير موجودة → ليس Admin
  ...
  // 3) Fetch registrations only for admin dashboard
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [user, view, isAdmin, adminChecked]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("حدث خطأ أثناء حفظ البيانات. (تحقق من الاتصال وقواعد Firestore)");
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      ...
      // بعد الدخول، التحقق من isAdmin سيتم تلقائياً في useEffect
      setView('dashboard');
    } catch (err) {
  ...
  const onLogout = async () => {
    await signOut(auth);
    // بعد تسجيل الخروج نرجع لتسجيل مجهول تلقائياً في أول useEffect عند reload
    setView('form');
    setLoginEmail('');
    setLoginPass('');
    setRegistrations([]);
  ...
  // ✅ Guard: if user enters dashboard but is not admin
  useEffect(() => {
    if (view !== 'dashboard') return;
    if (!adminChecked) return;

    if (!isAdmin) {
      alert("غير مسموح لك بالولوج إلى لوحة التحكم.");
      setView('form');
    }
  }, [view, isAdmin, adminChecked]);
  ...
  return (
    <>
      ...
      {view === 'dashboard' && adminChecked && isAdmin && (
        <DashboardView
          sidebarOpen={sidebarOpen}
          setSidebarOpen={setSidebarOpen}
          setView={setView}
          adminEmail={loginEmail || (auth.currentUser?.email ?? 'admin')}
